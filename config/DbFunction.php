...
class DbFunction{
	...
	function create_course($nom,$prenom,$sexe){
		
				if($nom==""){
			 
			echo "<script>alert('Select Name')</script>";
			...
			echo "<script>alert('Select  prenom')</script>";
		
		}
		else if($sexe==""){

			echo "<script>alert('Select  sexe')</script>";
		}
			...
			$db = Database::getInstance();
			$mysqli = $db->getConnection();
			$query = "insert into elèves(nom,prenom,sexe)values(?,?,?)";
			$stmt= $mysqli->prepare($query);
			if(false===$stmt){
				...
				$stmt->bind_param('sss',$nom,$prenom,$sexe);
				$stmt->execute();
				echo "<script>alert('Eleve Added Successfully')</script>";
...
function showCourse1($cid){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM elèves  where cid='".$cid."'";
	$stmt= $mysqli->query($query);
	return $stmt;
...
function edit_course($cshort,$cfull,$udate,$id){
	...
	$query = "update elèves set cshort=?,cfull=? ,update_date=? where cid=?";
	$stmt= $mysqli->prepare($query);
...
function del_course($id){
   ...
   //  echo $id;exit;
    $db = Database::getInstance();
    $mysqli = $db->getConnection();
    $query="delete from elèves where cid=?";
    $stmt= $mysqli->prepare($query);
